...
for name in ["bench_typical", "bench_xlarge", "bench_50x50"]
    ...
    big = n > 1_000_000
    ntr = big ? 2 : 3
    walk = big ? nothing : timed(() -> find_consistent(cib; exhaustive=false); n=ntr)
    exh  = timed(() -> find_consistent(cib; exhaustive=true); n=ntr)
    bas  = timed(() -> find_basins(cib); n=ntr)
    k    = length(find_consistent(cib; exhaustive=true))
    wk   = walk === nothing ? "  skip  " : string(round(walk, digits=6)) * "s"
    println("$name (n=$n, kernel=$k):  walk_1t=$wk  exhaustive=$(round(exh,digits=6))s  basins=$(round(bas,digits=6))s")
end
